...
public class SupplierMBean implements Serializable {
    ...
    // For detail view
    private Supplier selectedSupplierForDetail;
    ...
    // Lấy danh sách brands của supplier
    public List<Brand> getBrandsForSupplier(Supplier supplier) {
        if (supplier == null || supplier.getBrandCollection() == null) {
            return new ArrayList<>();
        }
        return new ArrayList<>(supplier.getBrandCollection());
    }
    
    // Đếm số lượng brands
    public int getBrandCount(Supplier supplier) {
        if (supplier == null || supplier.getBrandCollection() == null) {
            return 0;
        }
        return supplier.getBrandCollection().size();
    }
    
    // Lấy danh sách products từ các brands của supplier
    public List<Product> getProductsForSupplier(Supplier supplier) {
        ...
        for (Brand brand : supplier.getBrandCollection()) {
            if (brand != null && brand.getProductCollection() != null) {
                allProducts.addAll(brand.getProductCollection());
            }
        }
        return allProducts;
    }
    ...
    public int getProductCount(Supplier supplier) {
        return getProductsForSupplier(supplier).size();
    }
    ...
    // Xem chi tiết brands và products của supplier
    ...
    public Supplier getSelectedSupplierForDetail() {
        return selectedSupplierForDetail;
    }
